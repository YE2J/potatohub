...
**Helping set up the fleet.** When no existing profile fits, you can help the user
create one. See `references/multi-agent-fleet-setup.md` for the full setup workflow:
create profiles, configure per-profile models/providers, write role-defining SOUL.md,
initialize the kanban board, and wire the dispatcher. The reference covers the three-profile
pattern (orchestrator + N workers on different models) and every pitfall encountered.
