...
| `hermes -p worker-minimax chat` | ~42s | **MiniMax-M3** ✅ | 完整深度审计，含浮点精度、场景分级等高阶分析 | 重要审计：回测报告、跨文件一致性、方案复核 |
...
```bash
hermes -p worker-minimax chat -q "审计以下内容，按四维框架输出审计报告：
[粘贴待审材料]"
```
...
## worker-minimax profile 维护

```bash
# 连通性测试
hermes -p worker-minimax chat -q "只回复OK"

# API key 更新（当 MiniMax 换 key 时）
# .env 从 default profile 继承，更新 default 的 .env 后重新 clone：
hermes profile delete worker-minimax
...
2. worker-minimax 首次启动较慢（~40s 含模型元数据加载），后续调用会更快
